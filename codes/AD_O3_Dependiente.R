...
datos <- na.omit(datos)
datos$AQI_O3_cualitativo <- factor(datos$AQI_O3_cualitativo)
datos$AQI_O3_cualitativo <- factor(datos$AQI_O3_cualitativo, levels = c(0,1,2,3,4,5), labels = c("good", "moderate", "unhealthy fsg" , "unhealthy" , "very unhealthy","hazardous"))
library(caret)
library(MASS)
attach(datos)

tapply(O3, AQI_O3_cualitativo, mean)


index <- createDataPartition(datos$AQI_O3_cualitativo, p = 0.8, list = F)
...
modelo.train <- lda(AQI_O3_cualitativo ~ PM2_5 + PM10 + NO + NO2 + NOx + SR + RH + TOUT + PRS + WS + WD + CO, training)
...
tab <- table(Predicted = p1, Actual = test$AQI_O3_cualitativo)
tab
